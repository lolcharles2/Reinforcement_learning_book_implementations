...
class Agent:
    ...
    def __init__(self, Car, NN, state_to_basis):
        """ Initializes the agent.

            @type Car: RaceCar
                The RaceCar environment.
            @type NN: NeuralNet
                Neural network for computing the state-action value.
    ...
    def chooseAction(self, x, y, epsilon):
        """ Chooses action according to an epsilon greedy policy.
            ...
            @rtype: tuple
                A tuple (action, done), where action is an
                integer representing the action, and done is a boolean
                representing if the episode is finished.
        """

        if random.uniform(0, 1) < epsilon:
            return random.randint(0, 8), False
        else:
            with torch.no_grad():
                _, action = torch.max(self.NN(self.state_to_basis[(x,y)]), 1)
                return int(action.item()), True

    def oneStep(self, state, action, next_state, next_action, z, alpha, rho, lmbda, done):
        """ Performs one training step on observed transition.
            ...
            @type next_state: tuple[int]
                A tuple (x,y) denoting the next position.
            @type next_action: int
                An integer from 0-8 representing the next action.
            @type z: dict
                A dictionary that maps model parameters to their
                eligibility traces.
            @type alpha: float
                The step size.
            @type rho: float
            ...
            @type lmbda: float
                The parameter in SARSA(lambda).
            @type done: boolean
                Whether the episode has finished at next_state.
            @rtype: dict
                The updated eligibility trace dictionary.
        """
        ...
        with torch.no_grad():
            target = -1 if done else -1 + self.NN(self.state_to_basis[next_state])[0][next_action]
            delta = target - pred[0][action]
            for p in self.NN.parameters():
                z[p] = rho * lmbda * z[p] + p.grad
                p.copy_(p + alpha * delta * z[p])

        return z


    def train(self, episodes, epsilon, alpha, lmbda):
        """ Trains the agent using off-policy semi-gradient SARSA(lambda).
        ...
        for episode in range(episodes):
            ...
            if (episode + 1) % 1000 == 0:
                ...
                self.plotPolicy()
                torch.save(self.NN.state_dict(), 'race_car_NN_model_ET')
                print(f'Average steps per episode: {tot_steps/1000}')
                tot_steps = 0

            x, y, done = random.randint(0, self.Car.L1), 0, False
            action, greedy = self.chooseAction(x, y, epsilon)

            # Eligibility trace
            z = {}
            with torch.no_grad():
                for p in self.NN.parameters():
                    z[p] = torch.zeros(size=p.data.size()).to(device)

            while not done:

                tot_steps += 1

                new_x, new_y, done = self.Car.getNextState(x, y, action)

                # Importance sampling ratio
                rho = 0 if not greedy else 1 / (1 - epsilon)

                new_action, new_greedy = self.chooseAction(new_x, new_y, epsilon)

                z = self.oneStep((x,y), action, (new_x, new_y), new_action, z, alpha, rho, lmbda, done)

                x, y, action, greedy = new_x, new_y, new_action, new_greedy
...
if __name__ == "__main__":
    ...
    model = nn.Linear((L1+1)*(W1+W2+1)+L2*(W2+1), 9, bias=False).to(device)

    #model.load_state_dict(torch.load('race_car_NN_model_ET'))

    car_agent = Agent(Car, model, state_to_basis)

    car_agent.train(episodes, epsilon, alpha, lmbda)
